...
| Troca de senha | exige a senha atual. Tudo é lido com a chave antiga e selado com a nova **antes** de gravar: senha errada ou arquivo ilegível não alteram nada. O cofre é gravado por último, e as cópias em `backups/` acompanham a senha nova. `.canto` exportados antes continuam com a senha antiga |
