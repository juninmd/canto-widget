  ...
  return (
    ...
    >
      <div className="flex items-center justify-between text-muted">
        <span>vincular a uma tarefa ou evento de hoje</span>
        <button type="button" onClick={onClose} className="min-h-6 px-1 hover:text-fg">
          fechar
        </button>
      </div>
      {open.length === 0 && agenda.length === 0 && <p className="px-1 py-1 text-faint">nada para hoje ainda</p>}
      <ul className="max-h-32 overflow-y-auto">
        {open.map((t) => (
          <li key={t.id}>
            <button
              type="button"
              onClick={() => onPick({ kind: "task", id: t.id, label: t.title })}
