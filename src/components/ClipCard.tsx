...
export default function ClipCard({ item: i, copied, className, onCopy, onPin, onDelete }: Props) {
    ...
    <li className={`group rounded-lg border bg-ink/60 p-2 ${i.pinned ? "border-accent/60" : "border-edge hover:border-line"} ${className}`}>
      <button type="button" className="w-full text-left" title={copyTitle} onClick={onCopy}>
        {kind === "color" ? (
          ...
          <p className={`line-clamp-3 whitespace-pre-wrap text-xs ${kind === "link" ? "break-all text-accent" : "break-words text-fg"}`}>
            {i.preview}
          </p>
        )}
      </button>
