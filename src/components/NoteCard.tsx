...
export default function NoteCard({ note: n, className, onOpen, onPin, onDelete, onTag }: Props) {
    ...
    <li className={`group rounded-lg border bg-ink/60 p-2 ${n.fixada ? "border-accent/60" : "border-edge"} ${className}`}>
      <div className="flex items-start justify-between gap-2">
        <button type="button" className="min-w-0 flex-1 text-left" onClick={onOpen}>
          ...
          <p className="mt-0.5 line-clamp-3 whitespace-pre-line break-words text-xs text-muted">{n.body}</p>
        </button>
        <button
          type="button"
          onClick={onPin}
